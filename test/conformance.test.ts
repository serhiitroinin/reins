...
describe("adapter conformance", () => {
  test("the reference fixture passes the black-box contract", async () => {
    ...
    expect(report.cases.map((entry) => entry.name)).toEqual([
      ...
      "active subagent stop unsupported",
      "active subagent stop accepted",
      "active subagent stop false",
      "active subagent stop safe error",
      "active subagent stop cancellation race",
